...
void sgtl5000_triangle(h_sgtl5000_t *h_sgtl5000, float freq, int half)
{
    static float phase = 0.0f;
    const float fs = (float)AUDIO_FS;
    const float step = freq / fs;

    // Taille d'une moitié du buffer complet (stéréo)
      int half_size = AUDIO_BUFFER_LENGTH;   // = LENGTH * 2
      int start = half ? half_size : 0;
    ...
    for (int i = start; i < end; i++)
       {
           ...
           int16_t sample = (int16_t)(tri * 30000.0f);

           h_sgtl5000->sai_tx_buffer[2*i]   = sample;
           h_sgtl5000->sai_tx_buffer[2*i+1] = sample;
       }
}
